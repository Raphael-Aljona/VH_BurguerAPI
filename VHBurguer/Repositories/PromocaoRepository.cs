...
namespace VHBurguer.Repositories
{
    public class PromocaoRepository : IPromocaoRepository
    {
        ...
        {
            Promocao? promocao = _context.Promocao.FirstOrDefault(promocaoId => promocaoId.PromocaoID == id);
            return promocao!;
        }

        public bool NomeExiste(string nome, int? idAtual)
        {
            var consulta = _context.Promocao.AsQueryable();

            if (idAtual.HasValue)
            {
                consulta = consulta.Where(p => p.PromocaoID != idAtual.Value);
            }
